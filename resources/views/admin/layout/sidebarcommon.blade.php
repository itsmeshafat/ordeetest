<ul class="navbar-nav mx-3">
    ...
    App\Models\SystemAddons::where('unique_identifier', 'subscription')->first()->activated == 1)
    <li class="nav-item mb-2 fs-7">
        <a class="nav-link d-flex align-items-center  {{ request()->is('admin/industries') ? 'active' : '' }}" aria-current="page" href="{{ URL::to('admin/industries') }}">

            <span class="{{ request()->is('admin/industries') ? 'sidebariconbox' : 'sidebariconbox1' }}">
                <!-- <i class="fa-solid fa-money-check-dollar-pen"></i> -->
                <i class="fa-solid fa-hand-holding-dollar"></i>

            </span>
            <span class="px-2">{{ trans('Industries') }}</span>
        </a>
    </li>
    @endif
    @if (Auth::user()->type == 2)
